...
template <typename T> static int readString(T &s, std::istream &in) {
  uint32_t strSizeN;
...
template <typename T> static int writeString(const T &s, std::ostream &out) {
...
int TSDBEngineSample::executeTimeRangeQuery(const TimeRangeQueryRequest &trReadReq, std::vector<Row> &trReadRes) {
  mutex.lock();
  ...
  for (; rowIt != tableIt->second.cend(); ++rowIt) {
    const Row &curRow = *rowIt;
    if (curRow.vin == trReadReq.vin && curRow.timestamp >= trReadReq.timeLowerBound && curRow.timestamp < trReadReq.timeUpperBound) {
