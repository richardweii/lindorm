...
constexpr int kShardNum = 1 << 4; // 按照vin进行分片的数量，最好保证和kVinNum是整除的关系，这样每个分片的vin数量是均匀的
...
inline int Shard(uint16_t vid) { return vid % kShardNum; }
inline int vid2idx(uint16_t vid) { return vid >> kShardBits; }
inline int idx2vid(int shard_id, int idx) { return shard_id + (idx << kShardBits); }
