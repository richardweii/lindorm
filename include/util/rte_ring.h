...
public:
  ...
  Status enqueue(T *obj) {
    if (mode & RteRingMode::MP)
      return enqueue_mp(obj);
    else
      return enqueue_sp(obj);
  }

  Status dequeue(T **obj) {
    if (mode & RteRingMode::MC)
      return dequeue_mc(obj);
    else
      return dequeue_sc(obj);
  }
...
private:
  Status enqueue_sp(T *obj) {
    ...
    objs_[(prod_head) &mask] = obj;

    prod.prod_tail_ = prod_next;
    return RTE_RING_OK;
  }

  Status enqueue_mp(T *obj) {
    uint32_t prod_head, cons_tail;
    uint32_t prod_next, free_entries;
    unsigned rep = 0;
    ...
    do {
      ...
      prod_next = prod_head + 1;
      if (__atomic_compare_exchange_n(&prod.prod_head_, &prod_head, prod_next, false, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED)) {
        break;
      }
    } while (true);
  ...
  Status enqueue_sp_bulk(uint32_t n, T **obj_table, uint32_t *res_n, bool fixed) {
    ...
    if (UNLIKELY(n > free_entries)) {
      if (UNLIKELY(fixed || free_entries == 0))
        return RTX_RING_FULL;
      n = free_entries;
    }
  ...
  Status dequeue_mc(T **obj_table) {
    ...
    do {
      ...
      if (__atomic_compare_exchange_n(&cons.cons_head_, &cons_head, cons_next, false, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED)) {
        break;
      }
    } while (true);
