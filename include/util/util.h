...
inline void findMatchingIndices(uint16_t svid[], int64_t ts[], uint16_t idx[], int size, int target_vid,
                                int64_t ts_lower, int64_t ts_upper, std::vector<uint16_t>& idxs,
  ...
  if (vid_index < size) {
    ...
    while (start_idx > 0 && svid[start_idx - 1] == target_vid) {
      start_idx--;
    }
    int end_idx = vid_index;
    while (end_idx < size - 1 && svid[end_idx + 1] == target_vid) {
      end_idx++;
    }
