...
ColumnValue::ColumnValue(double_t value) : columnType(COLUMN_TYPE_DOUBLE_FLOAT) {
  columnData = (char *) malloc(sizeof(double_t));
  *((double_t *) columnData) = value;
}

ColumnValue::ColumnValue(const char *valuePtr, int32_t valueLength) : columnType(COLUMN_TYPE_STRING) {
...
ColumnValue::ColumnValue(const std::string &value) : columnType(COLUMN_TYPE_STRING) {
...
ColumnValue::ColumnValue(const std::vector<char> &value) : columnType(COLUMN_TYPE_STRING) {
  columnData = (char *) malloc(sizeof(int32_t) + value.size());
  *((int32_t *) columnData) = (int32_t) value.size();
  std::memcpy(columnData + sizeof(int32_t), value.data(), value.size());
}
...
bool ColumnValue::operator==(const ColumnValue &rhs) const {
  ...
  if (columnType == COLUMN_TYPE_STRING) {
    ...
    return lhsRawSize == rhsRawSize && std::strncmp(columnData + sizeof(int32_t), rhs.columnData + sizeof(int32_t), lhsRawSize - sizeof(int32_t)) == 0;
  }
  return false;
}
