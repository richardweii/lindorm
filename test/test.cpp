 ...
 * This test relies on the schemas object of TSDBEngineImpl class, since
...
static int verifyTableSchema(LindormContest::TSDBEngineImpl *engine);
...
static int schemaVerificationTask(const std::string &dataPath) {
  ...
  ret = verifyTableSchema((LindormContest::TSDBEngineImpl *) engine);
  if (ret != 0) {
...
static int verifyTableData(LindormContest::TSDBEngine *engine) {
  ...
  if (r0.columns["t1c3"].getStringValue(strBuff) != 0 || strBuff.first != 19 || std::strncmp(strBuff.second, str2, 19) != 0) {
    std::cerr << "RES incorrect" << std::endl;
    return -1;
  }
  if (r1.vin != vin2) {
  ...
  if (r1.columns["t1c3"].getStringValue(strBuff) != 0 || strBuff.first != 20 || std::strncmp(strBuff.second, str1, 20) != 0) {
    std::cerr << "RES incorrect" << std::endl;
    return -1;
  }
  ...
  if (trReadRes[0].vin != vin1 || trReadRes[0].timestamp != 1 || trReadRes[0].columns.size() != 1 ||
...
static int verifyTableSchema(LindormContest::TSDBEngineImpl *engine) {
  ...
  //   return -1;
  // }
  // if (schema1.columnTypeMap["t1c1"] != LindormContest::COLUMN_TYPE_INTEGER || schema1.columnTypeMap["t1c2"] != LindormContest::COLUMN_TYPE_DOUBLE_FLOAT || schema1.columnTypeMap["t1c3"] != LindormContest::COLUMN_TYPE_STRING) {
  //   std::cerr << "Table 1 schema verification failed" << std::endl;
  //   return -1;
  // }
  // if (schema2.columnTypeMap["t2c1"] != LindormContest::COLUMN_TYPE_DOUBLE_FLOAT || schema2.columnTypeMap["t2c2"] != LindormContest::COLUMN_TYPE_STRING || schema2.columnTypeMap["t2c3"] != LindormContest::COLUMN_TYPE_INTEGER) {
